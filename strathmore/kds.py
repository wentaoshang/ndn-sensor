...
class VerificationClosure(pyccn.Closure):
    def __init__(self, roster, key, timestamp, dsk, dsk_si):
        ...
        self.anchors = [{'name' : '/ndn/ucla.edu/bms/users/%C1.M.K%00%FF%B3%A0%01%12%5D%EF%CC%F3%00%16c%0E%A36%0D%8Bq%87%90t%1E%2C%29%0F%BC%FEMu%F69j',
                         'namespace' : '/ndn/ucla.edu/bms/users',
                         'pubkey' : \
	...
	{ 'key_pat': re.compile("^(/ndn/ucla.edu/bms/users)/%C1.M.K[^/]+$"), 
          'key_pat_ext': 0, 
          'data_pat': re.compile("^(/ndn/ucla.edu/bms/users(?:/[^/]+)*)$"), 
          'data_pat_ext': 0 }
        ]
        self.stack = []
