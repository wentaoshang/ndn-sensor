...
prefix = pyccn.Name(["wentao.shang","logtest"])
key = pyccn.Key()
key.fromPEM(filename = keyFile)
keyName = prefix.append("Key").appendKeyID(key).appendVersion().appendSegment(0)
si = pyccn.SignedInfo(key.publicKeyID, pyccn.KeyLocator(keyName), freshness = 1200)
